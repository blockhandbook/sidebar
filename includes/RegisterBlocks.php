...
class RegisterBlocks {
	...
	public function register_blocks() {
		...
		wp_register_style(
			SIDEBAR_PLUGIN_SLUG . '-block',
			SIDEBAR_PLUGIN_URL . 'build/style-index.css',
			array(),
		...
		wp_register_style(
			...
			SIDEBAR_PLUGIN_URL . 'build/index.css',
		...
		$script_asset_path = SIDEBAR_PLUGIN_PATH . 'build/index.asset.php';
		if ( ! file_exists( $script_asset_path ) ) {
			throw new \Error(
				'You need to run `npm start` or `npm run build` for the "sidebar" blocks first.'
			);
		}
		...
		wp_register_script(
			SIDEBAR_PLUGIN_SLUG . '-block-editor',
			SIDEBAR_PLUGIN_URL . 'build/index.js',
		...
		$blocks            = array();
		$block_directories = scandir( SIDEBAR_PLUGIN_PATH . 'src/blocks/' );
		...
		foreach ( $block_directories as $block ) {
			...
			register_block_type(
				SIDEBAR_PLUGIN_SLUG . "/$block",
				array(
					'style'         => SIDEBAR_PLUGIN_SLUG . '-block',
					'editor_style'  => SIDEBAR_PLUGIN_SLUG . '-block-editor',
					'editor_script' => SIDEBAR_PLUGIN_SLUG . '-block-editor',
				)
			);
		};
	}
